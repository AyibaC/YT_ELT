    ...
    try:
        if schema == 'staging':
            ...
            cur.execute( 
                ...
                "Video_Views",
                "Likes_Count",
                "Comments_Count") VALUES (%(video_id)s, %(title)s, %(publishedAt)s, %(duration)s, %(viewCount)s, %(likeCount)s, %(commentCount)s); 
                """, row #row is the second argument of the execute method, showing that each insert is a specific row
            ) # syntax after VALUES in insert statement is the syntax for saying which keys in the json should be inserted into which column in the table

        else: 
            video_id = 'Video_ID' # this is the column of the table in the staging schema
        ...
        logger.error(f"Error inserting row tihe video id: {row[video_id]}")
        raise e
